...
| `otlpTrace.traceEndpoint` | Full OTLP HTTP trace URL, used as-is. Takes precedence over `endpoint`. |
...
| `LOONGSUITE_PILOT_OTLP_TRACES_ENDPOINT` | Signal-specific OTLP trace endpoint. |
| `LOONGSUITE_PILOT_OTLP_ENDPOINT` | Legacy OTLP trace endpoint. |
...
| `otlp[].traceEndpoint` | otlp | Full OTLP HTTP trace URL, used as-is. Takes precedence over `endpoint`. |
