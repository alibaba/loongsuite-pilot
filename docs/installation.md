...
| `--multimodal-mode <mode>` | Global multimodal upload mode: `none`, `input`, `output`, `tool`, or `both`. Agents are detected ∩ selected ∩ {`codex`, `qoder`}; the same mode is written to each. Omit the flag to leave existing `uploadMode` unchanged and skip probing. `none` writes `uploadMode: none` onto the same detected ∩ selected set. When a non-`none` mode actually enables an agent and `--sls-endpoint` / `--sls-project` / `--sls-logstore` / `--sls-api-key` are all set, write `multimodal.storage` from those four. Install / reinstall only; `upgrade` and `uninstall` reject the flag. Windows: `-MultimodalMode`. |
