...
| `--multimodal-mode <mode>` | Global multimodal upload mode: `none`, `input`, `output`, or `both`. Written to existing {`codex`, `qoder`} agent entries. Omit to leave existing config unchanged. Non-`none` requires `--sls-endpoint`, `--sls-project`, `--sls-logstore`, and `--sls-api-key`. Install / reinstall only. Windows: `-MultimodalMode`. |
