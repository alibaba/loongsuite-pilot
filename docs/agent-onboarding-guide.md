...
每个输入源由 `AgentDiscoveryService` 管理，状态机为：

```
Idle → Starting → Running → Stopping → Idle
```

Discovery 通过 fs.watch + 定时轮询检测 agent 可用性变化，驱动状态转换。

### Stop Reason 语义

当输入源停止时，`agent:stopped` 事件携带 `reason` 字段（类型 `AgentStopReason`）：

| Reason | 触发条件 | 告警级别 |
|--------|----------|----------|
| `disabled` | 配置关闭（enabled=false） | 无告警，仅 debug 日志 |
| `unavailable` | 可用性检查失败（socket 不存在、进程未运行等） | 无告警，仅 debug 日志 |
| `shutdown` | 服务正常关机（orchestrator.stop()） | 无告警，仅 debug 日志 |
| `unexpected` | 运行中输入因异常退出（processEntry catch） | L3 `INPUT_STOP_ALARM` |

只有 `unexpected` 会触发 `INPUT_STOP_ALARM`。其他原因属于预期行为，不产生告警。

### 可用性防抖（unavailableThreshold）

对于 socket/状态容易短暂抖动的输入源（如 Wukong），可在 detection entry 中配置 `unavailableThreshold`：
...
entries.push(
  this.inputManager.buildDetectionEntry(wukongInput, {
    ...
    unavailableThreshold: 3, // 连续 3 次不可用才停止
  }),
);
```

单次不可用不会触发 `agent:stopped` 事件，避免瞬时抖动产生误报。

### 接入新输入源时避免误报

1. **正确实现 `isAvailable()`**：只检查持久性条件（文件/目录存在、daemon 进程存活），不要检查瞬时网络状态
2. **评估是否需要防抖**：如果 `isAvailable()` 依赖外部进程（socket、CLI 命令），考虑设置 `unavailableThreshold: 2~3`
3. **不要在 `start()`/`collect()` 中抛出可恢复错误**：可恢复错误应在内部处理并返回空结果，只有不可恢复的异常才应让 catch 分支触发 `unexpected`
4. **`stop()` 必须幂等**：正常关机和异常停止都会调用 `stop()`，确保重复调用不报错
