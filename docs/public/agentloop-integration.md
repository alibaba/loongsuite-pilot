...
# LoongSuite Pilot AI Coding Agent 接入指南（AgentLoop）

本文介绍如何通过 LoongSuite Pilot 将开发者本机的 AI Coding Agent 数据接入目标平台（AgentLoop）。接入后，您可以查看 AI Coding Agent 的会话、模型调用、工具调用、Token 用量、日志和 Trace 数据，用于团队使用分析、审计和可观测分析。
...
1. 登录目标平台控制台（AgentLoop），选择目标工作空间。
2. 在左侧导航栏进入接入中心。
