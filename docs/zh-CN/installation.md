...
| `--multimodal-mode <mode>` | 全局多模态上传模式：`none`、`input`、`output` 或 `both`。写到已有的 {`codex`, `qoder`} agent 配置。省略则不改已有配置。非 `none` 必须同时带 `--sls-endpoint`、`--sls-project`、`--sls-logstore`、`--sls-api-key`。仅 `install` / 重装有效。Windows：`-MultimodalMode`。 |
