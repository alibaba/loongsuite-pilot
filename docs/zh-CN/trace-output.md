...
| `otlpTrace.traceEndpoint` | 完整的 OTLP HTTP Trace 地址，按原值使用；优先于 `endpoint`。 |
...
| `LOONGSUITE_PILOT_OTLP_TRACES_ENDPOINT` | 按信号配置的 OTLP Trace endpoint。 |
| `LOONGSUITE_PILOT_OTLP_ENDPOINT` | 兼容旧配置的 OTLP Trace endpoint。 |
...
| `otlp[].traceEndpoint` | otlp | 完整的 OTLP HTTP Trace 地址，按原值使用；优先于 `endpoint`。 |
