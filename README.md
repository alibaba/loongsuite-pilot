...
Enabling both `upstreamLink.enabled` and `propagateToLlm` extends the same trace to the **LLM gateway**: Claude Code's fetch preload injects a `traceparent` header on outgoing `/v1/messages` requests, passing trace-id and flags through untouched and replacing parent-id with the span id of that LLM call, so gateway-side spans nest under it. A `traceparent` the caller already set is never overwritten, and the whole path is fail-open.
