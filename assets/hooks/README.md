...
own parsing, source-specific extraction, lightweight normalization, and writing
history/error/debug files.
...
`agent.raw` or an `agent.<source>.*` namespace.
...
or a clearly named `agent.raw` field so downstream policy can remove it.
...
2. Keep old raw/transcript output compatibility while existing inputs need it.
3. Update the matching `BaseHookInput` subclass to prefer canonical dotted keys
   and use old parsing only as fallback.
4. Add tests for processor output and input fallback behavior.
5. Verify replay from history JSONL still produces the same `AgentActivityEntry`
   semantics.
