...
# Copilot hook entrypoint — delegates to copilot/copilot-hook-processor.mjs.
#
# Usage (registered in ~/.copilot/config.json by pilot HookStrategy):
#   $PILOT_DATA/hooks/copilot-loongsuite-pilot-hook.sh <subcommand>
#
# Subcommand 与 Copilot CLI hook event 一一对应:
...
PROCESSOR="$SCRIPT_DIR/copilot/copilot-hook-processor.mjs"
...
log_error() {
  ...
  [[ -n "$day" ]] || day="unknown"
  local dir="$data_dir/logs/copilot/errors"
  local file="$dir/copilot-error-$day.jsonl"
  mkdir -p "$dir" 2>/dev/null || return 0
  printf '{"time":"%s","gen_ai.agent.type":"copilot","stage":"%s","error.type":"shell_%s","error.message":%s}\n' \
...
if [[ ! -f "$PROCESSOR" ]]; then
  echo "[copilot-hook] processor not found: $PROCESSOR" >&2
...
if [[ -z "$NODE_BIN" ]]; then
  echo "[copilot-hook] node >= $MIN_NODE_MAJOR not found" >&2
...
if ! "$NODE_BIN" "$PROCESSOR" "$SUBCOMMAND"; then
  echo "[copilot-hook] processor failed (subcommand=$SUBCOMMAND)" >&2
