...
function takeByName(items, used, name) {
  ...
  let index = items.findIndex((item, idx) => !used.has(idx)
    && normalizeToolName(item?.name ?? item?.toolName) === normalizedName);
  if (index < 0) index = items.findIndex((_item, idx) => !used.has(idx));
  ...
  const fusedCalls = llmCalls.map((call, callIndex) => {
    ...
    const tools = declaredIds.map((declaredId, toolIndex) => {
      ...
      if (completionIndex < 0) {
        completionIndex = groupUpdateCompletions.findIndex(
          (_completion, idx) => !localCompletionUsed.has(idx),
        );
      }
    ...
    return {
      ...
      finishReason,
      tools,
      input_tokens: group?.promptTokens ?? call.input_tokens ?? 0,
      output_tokens: group?.completionTokens ?? call.output_tokens ?? 0,
      cache_read_input_tokens: group?.cachedPromptTokens ?? call.cache_read_input_tokens ?? 0,
