...
export function parseGrokUnified(filePath, sessionId, options = {}) {
  ...
  for (const record of records) {
    ...
    if (record.msg === 'shell.turn.inference_done') {
      ...
      target.promptTokens = numberOrNull(ctx.prompt_tokens) ?? 0;
      target.cachedPromptTokens = numberOrNull(ctx.cached_prompt_tokens) ?? 0;
      target.cacheCreationInputTokens = numberOrNull(ctx.cache_creation_input_tokens) ?? 0;
      target.completionTokens = numberOrNull(ctx.completion_tokens) ?? 0;
      target.reasoningTokens = numberOrNull(ctx.reasoning_tokens) ?? 0;
      current = target;
      continue;
    }
