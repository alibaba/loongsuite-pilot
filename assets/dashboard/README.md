...
On the first visit, the page selects Simplified Chinese when any browser
language starts with `zh`; otherwise it uses English. The language selector in
the header applies immediately to labels, states, errors, accessibility text,
numbers, dates, and times. The choice is saved in `localStorage` when available;
the dashboard continues to work when browser storage is blocked.
