# Loongpilot Dashboard

This lightweight local dashboard shows Loongpilot collection and reporting health.

## Start

```bash
loongpilot monitor-start
```
...
The monitor is optional. It includes both the process sampler and the local dashboard server. `loongpilot start` starts the core collector only; it does not automatically start monitor.
...
Stop it without stopping Loongpilot collection/reporting:

```bash
loongpilot monitor-stop
```

`loongpilot stop` stops the optional monitor if it is running. Generated Process Resources CSV files are kept according to the monitor retention policy.

## Status Meanings

- `Active`: Loongpilot has processed recent events for this agent.
...
- The monitor writes hourly CSV files named `loongpilot-process-YYYY-MM-DD-HH.csv`.
- The dashboard reads the last 60 minutes by default.
- Old hourly metrics files are deleted after `LOONGPILOT_MONITOR_RETENTION_HOURS`, default `6`.
...
- `LOONGPILOT_MONITOR_WINDOW_MINUTES`: dashboard metrics window, default `60`.
- `LOONGPILOT_MONITOR_RETENTION_HOURS`: on-disk metrics retention, default `6`.
- `LOONGPILOT_MONITOR_CLEANUP_INTERVAL_SECONDS`: cleanup interval, default `300`.
