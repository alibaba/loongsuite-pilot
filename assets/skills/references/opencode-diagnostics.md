...
`~/.loongsuite-pilot/skills/references/opencode-diagnostics.md`，随 pilot 升级自动更新。
...
tail -2 ~/.loongsuite-pilot/logs/opencode/opencode-$(date +%Y-%m-%d).jsonl \
  | python3 -m json.tool
```
...
tail -2 ~/.loongsuite-pilot/logs/output/opencode-$(date +%Y-%m-%d).jsonl 2>/dev/null
```
