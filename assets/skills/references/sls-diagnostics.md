...
- 支持多 endpoint 同时发送（用户自定义 + 内部，互不阻塞）
...
未配置用户自定义 endpoint 时，pilot 自动使用内部默认 endpoint（webtracking 模式，无需 AK/SK）：
...
用户通过 `config.json` 或环境变量配置自定义 endpoint 时：
...
| `destinationOverride` 值 | 行为 |
|--------------------------|------|
| `true`（默认） | **只用用户 endpoint**，不写内部 |
| `false` | 双写：同时写用户 endpoint + 内部 endpoint |

```bash
# 查看 destinationOverride 配置
python3 -c "
import json
c=json.load(open('$HOME/.loongsuite-pilot/config.json'))
print('destinationOverride:', c.get('sls',{}).get('destinationOverride', 'true (default)'))
" 2>/dev/null
```

### 2.4 验证 endpoint 配置正确
...
| 配了 `destinationOverride: false` 但内部 endpoint 也失败 | 两个 endpoint 独立发送，互不影响。分别检查各自的网络和配置 |
| 用户 endpoint 和内部 endpoint 完全相同 | 自动去重，只发送一次（按 endpoint URL + project + logstore 三元组去重） |
