...
The collector's current `AgentActivityEntry` shape uses a partial OTel style with shortened fields such as `session.id`, `turn.id`, `agent.type`, `message.role`, `cost.*`, and `tool.result.*`. The current endpoint-side Agent schema in `端侧 Agent 支持情况 (1).md` defines a different canonical reporting contract in section 3, so the OpenSpec change and implementation plan need to align with that table before code changes begin.

## What Changes

- Standardize `AgentActivityEntry` on the field definitions from section 3 of the current endpoint-side Agent schema.
- Treat OTel-style top-level fields as canonical where listed: `time_unix_nano`, `observed_time_unix_nano`, `event.*`, `user.id`, `trace_id`, `span_id`, `parent_span_id`, `host.*`, `service.name`, `error.*`, and `is_error`.
- Move Agent, message, request, response, usage, cost, tool, and skill fields into their canonical `gen_ai.*` names, including `gen_ai.turn.id`, `gen_ai.step.id`, `gen_ai.agent.*`, `gen_ai.message.role`, `gen_ai.request.id`, `gen_ai.usage.*_cost`, `gen_ai.tool.call.exec.id`, and `gen_ai.tool.call.duration_ms`.
...
- Keep input compatibility for old local logs and existing hook/transcript records by accepting legacy aliases such as `session.id`, `turn.id`, `step.id`, `agent.type`, `message.role`, `request.id`, `provider.name`, `request.model`, `usage.input_tokens`, `cost.input`, `tool.exec.id`, `tool.result.duration_ms`, `input.messages`, `tool.arguments`, and `tool.result.payload`.
