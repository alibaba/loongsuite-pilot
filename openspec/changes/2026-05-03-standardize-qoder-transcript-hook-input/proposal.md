...
- Preserve the current hook installation and history-log compatibility path in this change:
  - Keep the existing `qoder-loongsuite-pilot-hook.sh` behavior and `logs/qoder-cli/history/qoder-cli-*.jsonl` channel unless a later migration explicitly renames it.
