...
`QoderSqliteInput` extends `BaseSqliteInput`, which reads rows using the persisted `lastRowId` cursor. When `lastRowId` is absent, the base class defaults to `0`, so a fresh LoongSuite Pilot install or deleted `~/.loongsuite-pilot/logs/input-state.json` causes `qoder-sqlite` to query every historical eligible `chat_message` row.
...
**Goals:**
- Prevent historical Qoder SQLite token usage replay when LoongSuite Pilot state is missing.
...
- Deleting LoongSuite Pilot state intentionally discards the ability to recover missed rows -> This matches the requested no-history behavior.
