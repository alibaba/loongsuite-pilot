...
let options;
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'pilot-dashboard-cli-shell-'));
...
describe('installer and shell integration', () => {
  it('dispatches to the current version with the pinned Node and exact config', () => {
    ...
    for (const version of ['old', 'new']) {
      ...
      for (const args of [['url'], ['shortcut', 'install'], ['shortcut', 'status'], ['shortcut', 'uninstall']]) {
        ...
        expect(JSON.parse(result)).toEqual({ version, config: options.configPath, args });
      }
    }
