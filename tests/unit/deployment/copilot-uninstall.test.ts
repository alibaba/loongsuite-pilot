...
it('Unix uninstall removes only Pilot Copilot registration, assets and environment',()=>{
 ...
 fs.writeFileSync(path.join(root,'config.json'),'// User settings belong in settings.json.\n/* Native CLI managed file */\n'+JSON.stringify({installedPlugins:[{name:'loongsuite-pilot',marketplace:'loongsuite-pilot'},{name:'third',marketplace:'third'}],url:'https://example.test/path//keep'}).replace(/}$/,',}'));
 ...
 expect(JSON.parse(fs.readFileSync(path.join(root,'config.json'),'utf8')).url).toBe('https://example.test/path//keep');
