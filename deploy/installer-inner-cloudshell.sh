...
DEFAULT_PACKAGE_URL="https://security-sdk.oss-cn-hangzhou.aliyuncs.com/agentshell/releases/lastest/macos/${PILOT_VERSION}/loongsuite-pilot.tar.gz"
...
PACKAGE_URL="${LOONGSUITE_PILOT_PACKAGE_URL:-$DEFAULT_PACKAGE_URL}"
INSTALL_VERSION="${PILOT_VERSION}"
SLS_ENDPOINT=""
SLS_PROJECT=""
SLS_LOGSTORE=""
SLS_AK_ID=""
...
resolve_node() {
    ...
    _candidates+=(
        ...
        /usr/local/bin/node
        "$HOME/.local/bin/node"
        "$HOME/.local/lib/agentshell-node/node-v22.23.1-darwin-arm64/bin/node"
        "$HOME/.local/lib/agentshell-node/node-v22.23.1-darwin-x64/bin/node"
    )
...
# ============================================================
# Common: bootstrap Node.js runtime from OSS when system Node is
# missing, too old, or architecture-mismatched. Downloads a fixed
# v22.23.1 build, verifies MD5, and installs to
# ~/.local/lib/agentshell-node/<pkg>/ with symlinks in ~/.local/bin/.
# Sets NODE_BIN and NPM_BIN on success.
# ============================================================
_NODE_BOOTSTRAP_BASE_URL="https://security-sdk.oss-cn-hangzhou.aliyuncs.com/agentshell/node"
_NODE_BOOTSTRAP_VERSION="22.23.1"

_detect_node_arch() {
    local sys_arch
    sys_arch=$(uname -m)
    # macOS: prefer hw.optional.arm64 to detect real hardware (avoid Rosetta)
...
bootstrap_node() {
    msg "==> 本地 Node 不符合要求，开始自举 Node.js v${_NODE_BOOTSTRAP_VERSION}..." \
        "==> Local Node unsuitable, bootstrapping Node.js v${_NODE_BOOTSTRAP_VERSION}..."

    local arch; arch=$(_detect_node_arch)
    if [ -z "$arch" ]; then
        msg "❌ 无法识别系统架构，无法自举 Node.js" \
            "❌ Cannot detect system architecture, unable to bootstrap Node.js"
        exit 1
    fi

    local pkg_dir="node-v${_NODE_BOOTSTRAP_VERSION}-darwin-${arch}"
    local pkg_file="${pkg_dir}.tar.xz"
    local pkg_url="${_NODE_BOOTSTRAP_BASE_URL}/${pkg_file}"
    local expected_md5=""
    case "$arch" in
        arm64) expected_md5="deb93bb10d3a4317c5cc3a2b5951fcad" ;;
        x64)   expected_md5="e59a3edd06a2d122934c6fcd25fa343f" ;;
    esac

    local tmp_dir
    tmp_dir=$(mktemp -d /tmp/agentshell-node.XXXXXX) || {
        msg "❌ 创建临时目录失败" "❌ Failed to create temp directory"
        exit 1
    }

    local archive_path="$tmp_dir/$pkg_file"

    msg "    下载 $pkg_url" "    Downloading $pkg_url"
    if command -v curl &>/dev/null; then
        curl -L -f -s -S --retry 3 --retry-delay 2 -o "$archive_path" "$pkg_url" || {
            msg "❌ Node 下载失败" "❌ Node download failed"
            rm -rf "$tmp_dir"
            exit 1
        }
    else
        wget -q --tries=3 -O "$archive_path" "$pkg_url" || {
            msg "❌ Node 下载失败" "❌ Node download failed"
            rm -rf "$tmp_dir"
            exit 1
        }
    ...
    if command -v md5sum &>/dev/null; then
        actual_md5=$(md5sum "$archive_path" 2>/dev/null | awk '{print $1}') || actual_md5=""
    elif command -v md5 &>/dev/null; then
        actual_md5=$(md5 -q "$archive_path" 2>/dev/null) || actual_md5=""
    else
        msg "❌ 找不到 md5sum 或 md5 工具，无法校验完整性" \
            "❌ Neither md5sum nor md5 found, cannot verify integrity"
        rm -rf "$tmp_dir"
        exit 1
    fi

    if [ -z "$actual_md5" ] || [ "$actual_md5" != "$expected_md5" ]; then
        msg "❌ MD5 校验失败: 期望 $expected_md5，实际 ${actual_md5:-<空>}" \
            "❌ MD5 mismatch: expected $expected_md5, got ${actual_md5:-<empty>}"
        rm -rf "$tmp_dir"
        exit 1
    fi
    msg "    ✅ MD5 校验通过" "    ✅ MD5 verified"

    # Extract
    msg "    解压 $pkg_file ..." "    Extracting $pkg_file ..."
    if ! tar -xf "$archive_path" -C "$tmp_dir"; then
        msg "❌ 解压失败" "❌ Extraction failed"
        rm -rf "$tmp_dir"
        exit 1
    fi

    # Verify extracted structure: bin/node, bin/npm, npm-cli.js
    local extracted_dir="$tmp_dir/$pkg_dir"
    if [ ! -f "$extracted_dir/bin/node" ] || \
       [ ! -x "$extracted_dir/bin/npm" ] || \
       [ ! -f "$extracted_dir/lib/node_modules/npm/bin/npm-cli.js" ]; then
        msg "❌ 解压后的 Node 包结构不完整" \
            "❌ Extracted Node package structure is incomplete"
        rm -rf "$tmp_dir"
        exit 1
    fi

    # Install to ~/.local/lib/agentshell-node/<pkg_dir>
    local install_base="$HOME/.local/lib/agentshell-node"
    local install_dir="$install_base/$pkg_dir"
    mkdir -p "$install_base"
    rm -rf "$install_dir"
    if ! mv "$extracted_dir" "$install_dir"; then
        msg "❌ 安装目录迁移失败" "❌ Failed to move to install directory"
        rm -rf "$tmp_dir"
        exit 1
    fi

    # Ensure executables are runnable
    chmod 755 "$install_dir/bin/node" 2>/dev/null || true
    chmod 755 "$install_dir/lib/node_modules/npm/bin/npm-cli.js" 2>/dev/null || true

    # Cleanup temp
    rm -rf "$tmp_dir"

    # Verify the installed node works
    if ! "$install_dir/bin/node" --version >/dev/null 2>&1; then
        msg "❌ 自举后的 Node 无法执行" "❌ Bootstrapped Node is not executable"
        exit 1
    fi

    msg "    ✅ Node 已安装到 $install_dir" "    ✅ Node installed to $install_dir"
    echo ""

    # Export for the rest of check_deps — use full path, no symlinks
...
check_deps() {
    ...
    # Clean up legacy symlinks created by previous installer versions
    # (pointed ~/.local/bin/{node,npm} → agentshell-node)
    local _bin_dir="$HOME/.local/bin"
    for _sym in node npm; do
        if [ -L "$_bin_dir/$_sym" ]; then
            local _target
            _target=$(readlink "$_bin_dir/$_sym" 2>/dev/null || true)
            if [[ "$_target" == *agentshell-node* ]]; then
                rm -f "$_bin_dir/$_sym"
                msg "    🧹 清理旧软链接: $_bin_dir/$_sym → $_target" \
                    "    Cleaned legacy symlink: $_bin_dir/$_sym → $_target"
            fi
        fi
    done

    local _bootstrapped=0

    NODE_BIN=$(resolve_node) || {
        msg "⚠️  未找到合格的 Node.js，开始自举 Node.js 运行时..." \
            "⚠️  No suitable Node.js found, bootstrapping Node.js runtime..."
        bootstrap_node
        _bootstrapped=1
    }
    ...
    if [ "$_bootstrapped" -eq 0 ]; then
        ...
        if [ "$NODE_MAJOR" -lt 18 ]; then
            msg "⚠️  Node 版本过低 ($("$NODE_BIN" --version))，开始自举..." \
                "⚠️  Node version too low ($("$NODE_BIN" --version)), bootstrapping..."
            bootstrap_node
            _bootstrapped=1
        fi
    fi
    ...
    # Ensure node's bin dir is on PATH so npm shebang (#!/usr/bin/env node)
    # resolves — critical under `sudo -u -H` which resets PATH via secure_path
    export PATH="$(dirname "$NODE_BIN"):$PATH"

    # Derive npm from the same installation (skip if just bootstrapped;
    # bootstrap_node already sets NPM_BIN)
    if [ "$_bootstrapped" -eq 0 ]; then
    ...
    if [ "$(uname)" = "Darwin" ]; then
        ...
        if [ "$sys_arch" = "arm64" ] && [ "$node_arch" = "x64" ]; then
            if [ "$_bootstrapped" -eq 0 ]; then
                msg "⚠️  架构不匹配: 系统为 arm64 (Apple Silicon)，但 Node.js 为 x64 (Intel)，开始自举..." \
                    "⚠️  Architecture mismatch: system is arm64 but Node.js is x64, bootstrapping..."
                bootstrap_node
                _bootstrapped=1
                # Re-pin after bootstrap
                echo "$NODE_BIN" > "$DATA_DIR/node-bin"
            else
                # Already bootstrapped — should not mismatch, warn only
                msg "⚠️  架构不匹配: 系统为 arm64 但 Node.js 为 x64" \
                    "⚠️  Architecture mismatch: system is arm64 but Node.js is x64"
...
deploy_package() {
    ...
    (cd "$PERMANENT_DIR" && "$NPM_BIN" install --production --no-optional --no-audit --no-fund 2>&1 | tail -1)
...
// Auto-update removed: strip stale auto-update config left by older versions.
delete config.autoUpdate;
delete config.canary;
delete config.installId;
...
const INTERNAL_SLS = {
  ...
  project: 'agentloop-dd104e7aa549dd712d1abb2a805b1dac',
  logstore: 'agent-event-webtracking',
  mode: 'webtracking',
};
...
const INTERNAL_CMS = {
  ...
  licenseKey: 'i5a2l7sk3h@e7317437b8f81ab',
  workspace: 'default-cms-1842974047653821-cn-zhangjiakou',
};
...
cmd_install() {
    ...
    if [ -n "$cur_ver" ]; then
        # Skip rules for existing installations:
        #   1. Current version contains "agentshell":
        #      a. Download version > installed → overwrite.
        #      b. Download version == installed → skip if service running,
        #         overwrite if not running.
        #      c. Download version < installed → skip.
        #      d. Download version unknown (e.g. "latest" URL) → overwrite.
        #   2. Current version does NOT contain "agentshell" → skip.
        if [[ "$cur_ver" == *agentshell* ]]; then
            local download_ver="$INSTALL_VERSION"
            local cmp=0
            version_compare "$download_ver" "$cur_ver" || cmp=$?
            case $cmp in
                1)  # download > installed → overwrite
                    msg "⚠️  检测到已安装 v${cur_ver}，下载版本 v${download_ver} 更高，执行覆盖安装" \
                        "⚠️  Existing v${cur_ver}, download v${download_ver} is newer, overwriting"
                    ;;
                0)  # download == installed → check running status
                    local _installed_cli="$HOME/.local/bin/loongsuite-pilot"
                    local _status_out=""
                    if [ -x "$_installed_cli" ]; then
                        _status_out=$("$_installed_cli" status 2>/dev/null || true)
                    fi
                    if printf '%s\n' "$_status_out" | grep -q "is running"; then
                        msg "✅ 检测到已安装 v${cur_ver} 且服务正在运行，跳过安装" \
                            "✅ Existing v${cur_ver} is running, skipping installation"
                        return 0
                    fi
                    msg "⚠️  检测到已安装 v${cur_ver} 且服务未运行，执行覆盖安装" \
                        "⚠️  Existing v${cur_ver} is not running, overwriting"
                    ;;
                2)  # download < installed → skip
                    msg "✅ 检测到已安装 v${cur_ver}，下载版本 v${download_ver} 较旧，跳过安装" \
                        "✅ Existing v${cur_ver} is newer than download v${download_ver}, skipping"
                    return 0
                    ;;
            esac
        else
    ...
    # Let the previously installed CLI unload its service manager jobs first.
    # A raw PID kill lets launchd KeepAlive race the installer and restart the
    # old collector/updater before the new CLI can replace the plist.
    local installed_cli="$HOME/.local/bin/loongsuite-pilot"
    if [ -x "$installed_cli" ]; then
        msg "==> 停止现有服务与自动启动项..." \
            "==> Stopping existing service and autostart jobs..."
        "$installed_cli" stop >/dev/null 2>&1 || true
    fi

    # Fallback for incomplete legacy installations without a working CLI.
    ...
    if loongsuite-pilot start $_start_args; then
        ...
        if printf '%s\n' "$status_output" | grep -q "is running"; then
            ...
            msg "    ❌ 服务启动验证失败，请检查: loongsuite-pilot status" \
                "    ❌ Service start verification failed. Check: loongsuite-pilot status"
            return 1
        fi
    else
        msg "    ❌ 服务启动失败，请检查服务日志" \
            "    ❌ Service failed to start. Check the service log."
        return 1
    fi
    echo ""
...
# ============================================================
# Remove shell-rc intercept wrappers injected by the collector
# (hook-watchdog writes `# loongsuite-pilot BEGIN <name>-intercept` blocks with
# claude()/qodercli() wrapper functions into ~/.zshrc / ~/.bashrc / ~/.bash_profile;
# without this cleanup they dangle after uninstall and break `claude`/`qodercli`
# because the preloaded hook file under ~/.loongsuite-pilot/hooks is gone).
# ============================================================
remove_shell_intercept_wrappers() {
    local cleaned=0
    for rc in "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"; do
        [ -f "$rc" ] || continue
        if grep -q "# loongsuite-pilot BEGIN" "$rc" 2>/dev/null; then
            sed -i.bak '/# loongsuite-pilot BEGIN/,/# loongsuite-pilot END/d' "$rc"
            rm -f "${rc}.bak"
            cleaned=1
        fi
    done
    # macOS: also tear down the QoderWork env launchd agent the collector installs
    # (sets QODER_WORKER_RUNTIME_PATH). It's separate from the collector service,
    # so `loongsuite-pilot stop` / autostart_remove never removes it.
    if [ "$(uname -s)" = "Darwin" ]; then
        local _qwplist="$HOME/Library/LaunchAgents/com.loongsuite-pilot.qoderwork-env.plist"
        if [ -f "$_qwplist" ]; then
            launchctl unload -w "$_qwplist" 2>/dev/null || true
            rm -f "$_qwplist"
            cleaned=1
        fi
    ...
    if [ "$cleaned" -eq 1 ]; then
        msg "    ✅ shell/agent 注入残留已清理 (重开终端或 source 生效)" \
            "    ✅ shell/agent injection residue cleaned (reopen terminal or source to apply)"
    fi
}
...
cmd_uninstall() {
    ...
    if command -v loongsuite-pilot &>/dev/null; then
        ...
        case "$(uname -s)" in
            Darwin)
                local _plist="$HOME/Library/LaunchAgents/com.agentshell.plist"
                local _legacy_plist="$HOME/Library/LaunchAgents/com.loongsuite-pilot.plist"
                local _uplist="$HOME/Library/LaunchAgents/com.loongsuite-pilot.updater.plist"
                for f in "$_uplist" "$_plist" "$_legacy_plist"; do
                    if [ -f "$f" ]; then
    ...
    # Remove loongsuite-pilot command (and the agentshell display-name wrapper)
    msg "==> 删除 loongsuite-pilot 命令..." "==> Removing loongsuite-pilot command..."
    rm -f "$HOME/.local/bin/loongsuite-pilot"
    rm -f "$HOME/.local/bin/agentshell"
